...
describe("1. AUTOINCREMENT sequence restore", () => {
  it("deleted top ids are not reused; sqlite_sequence has one exact row", async () => {
...
describe("2. rowid alias + indexes + trigger + view", () => {
  ...
  it("rowid alias in second column position migrates byte-exact", async () => {
...
describe("4. hostile KV entries", () => {
  it("all 199 keys migrate, including user keys starting with __claydo", async () => {
  ...
  it("an instance whose ONLY data is __claydo-prefixed user keys migrates fully", async () => {
